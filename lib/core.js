...
async function visionChat({ baseUrl, model, apiKey, source, question, maxTokens = 2048, timeoutMs = REQUEST_TIMEOUT_MS, maxImageBytes = MAX_BYTES, fetchImpl = fetch, signal }) {
  ...
  if (status >= 400) {
    ...
    throw new Error('视觉模型 API 返回 HTTP ' + status + (msg ? '：' + redact(msg) : '') + hint)
  }
  const text = extractText(parsed)
  if (!text) throw new Error('视觉模型返回了无法解析的响应：' + redact(body.slice(0, 500) || '(空)'))
  const stripped = stripThink(text)
  if (!stripped) throw new Error('视觉模型返回了空内容。')
  return stripped
}
