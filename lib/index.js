      ...
      const tool = {
        ...
        async execute(args, exec) {
          ...
          try {
            ...
            const model = await configValue(ctx, 'QWEN_VL_MODEL', core.DEFAULT_MODEL)
            ...
            const text = await core.visionChat({
              baseUrl, model, apiKey, source, question,
            ...
            return { text: '错误：视觉模型调用失败：' + core.redactText((e && e.message) || e, apiKey) }
          }
        },
      }
